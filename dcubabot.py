...
def sugerirNoticia(update, context):
	...
	try:
		...
		update.message.reply_text(text="Loc@, pusisiste algo mal, la idea es q pongas:\n /sugerirNoticia <texto>")
		return
	print("texto", texto)
	try:
		with db_session:
	...
	with db_session:
		...
		if buttonType == "Noticia":
			...
			if action == "1":
				...
				action_text = "\n¡Aceptado!"
				context.bot.sendMessage(chat_id="@NoticiasDC", text=noticia.text, parse_mode=ParseMode.MARKDOWN)
			else:
				noticia.delete()
				action_text = "\n¡Rechazado!"
			context.bot.editMessageText(chat_id=message.chat_id, message_id=message.message_id,
